...
class AccountingTransactionService{
    ...
    private client:AccountingTransactionClient;

    constructor(icpUrl:string,canisterId:string){
        this.client = new AccountingTransactionClient(icpUrl, canisterId);
    }

    async listAccountingTransactions(
        query: ListAccountingTransactionQuery,
    ): Promise<AccountingTransaction[]> {
        const tickets = await this.client.listTicketTransactions(query.dateFrom, query.dateTo);
        const invoices = await this.client.listInvoiceTransactions(query.dateFrom, query.dateTo);
        const bankings = await this.client.listBankTransactions(query.dateFrom, query.dateTo);
    ...
    async listTransactionIDs(
        ...
        const tickets = await this.client.getTransactionIds("ticket", query.dateFrom, query.dateTo);
        const invoices = await this.client.getTransactionIds("invoice", query.dateFrom, query.dateTo);
        const bankings = await this.client.getTransactionIds("bank", query.dateFrom, query.dateTo);
    ...
    async listSuppliersInvoicesByStore(store: StoreDto): Promise<InvoiceAccountingTransaction[]> {
        const transactions = await this.client.listInvoiceTransactions();
        return transactions.filter((transaction) => Number(transaction.Buyer.ID) === store.id);
    }
    ...
    async getTransactionsByIDs(
        ...
        const tickets = await this.client.getTransactionsByIds("ticket", ticketIds);
        const invoices = await this.client.getTransactionsByIds("invoice", invoiceIds);
        const bankings = await this.client.getTransactionsByIds("bank", bankIds);
    ...
    async getAccountingTransaction(
        ...
        switch (query.type) {
            case AccountingTransactionType.TICKET:
                return await this.client.getTicketTransactionById(id);
            case AccountingTransactionType.INVOICE:
                return await this.client.getInvoiceTransactionById(id);
            case AccountingTransactionType.BANK_TRX:
                return await this.client.getBankTransactionById(id);
            default:
                throw new Error('Unknown transaction type');
        }
    }
...
export default AccountingTransactionService;
